...
# --- MEMORY FUNCTION ADDED HERE ---
# 1. Read the saved memory from the URL (if it exists), otherwise use defaults
saved_portfolio = st.query_params.get("portfolio", default_portfolio)
saved_debt = float(st.query_params.get("debt", 0.0))
saved_rate = float(st.query_params.get("rate", 5.0))

portfolio_input = st.sidebar.text_area("Your Holdings:", value=saved_portfolio, height=150)

st.sidebar.header("⚖️ Leverage & Debt")
margin_debt = st.sidebar.number_input("Total Margin/Debt ($)", min_value=0.0, value=saved_debt, step=1000.0)
margin_interest_rate = st.sidebar.slider("Margin Interest Rate (%)", min_value=0.0, max_value=20.0, value=saved_rate, step=0.1)

# 2. Update the URL memory immediately whenever you type something new
...
with tab3:
    ...
    st.markdown("Project your equity growth over time. Each holding grows at its **individual Expected Yield** (automatically calculated from 1-year historical returns unless overridden).")
    
    g_col1, g_col2 = st.columns(2)
    years_to_project = g_col1.slider("Years to Project", min_value=1, max_value=30, value=10, step=1)
    annual_contribution = g_col2.number_input("Additional Annual Contribution ($)", min_value=0.0, value=12000.0, step=1000.0)
    
    if total_asset_value > 0:
        weighted_avg_cagr = (df_portfolio['Expected Yield (%)'] * df_portfolio['Total Value']).sum() / total_asset_value
        st.info(f"💡 **Projection Engine:** Individual holdings grow at their specific yields. Your unallocated cash (new contributions minus margin interest) is assumed to grow at your portfolio's current weighted average yield of **{weighted_avg_cagr:.2f}%**.")
        ...
        st.success(f"📈 After **{years_to_project} years**, contributing **${annual_contribution:,.0f}** yearly, your Net Equity is estimated to be **${projected_equity[-1]:,.2f}**.")
    else:
